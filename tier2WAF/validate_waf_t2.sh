...
response_code=$(curl -sku admin:$(passwd) -w "%{http_code}" -X PUT -H "Content-Type: application/json" https://localhost/mgmt/tm/sys/icall/handler/periodic/eventhub_datagroup -d '{"status": "inactive", "interval": "10",  "script": "eventhub_datagroup"}' -o /dev/null)
